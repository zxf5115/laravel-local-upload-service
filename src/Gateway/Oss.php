...
class Oss extends Common
{
  ...
  public static function file($name, $path = 'uploads', $allow = [], $is_filename = false)
  {
    try
    {
      ...
      // 上传文件到OSS服务器
      if(false !== $url = self::wirteOssServer($filename, $resource))
      {
        if($is_filename)
        {
          return [
  ...
  public static function file_base64($file, $path = 'uploads', $allow = [], $name = false)
  {
    try
    {
      ...
      // 创建目录
      Storage::disk('oss')->makeDirectory($dir);
      ...
      if(Storage::disk('public')->put($filename, $file))
      {
        ...
        // 将资源上传到OSS服务器
        $url = self::wirteOssServer($filename, $resource);
   ...
   * @return [type]
   */
  private static function wirteOssServer($filename, $resource)
  {
    try
    {
      $response = Storage::disk('oss')->writeStream($filename, $resource);

      if($response)
      {
        return  Storage::disk('oss')->url($filename);
      }
      else
      {
        return false;
